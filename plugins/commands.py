...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        files = files_[0]
        title = search(r'^(.*?\s\(\d{4}\))', ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and x not in ['mkv', 'mp4'], files.file_name.split()))).group(1)
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{' '.join([x for x in files.file_name.split() if not x.startswith('[') and not x.startswith('@') and x not in ['mkv', 'mp4']])}"
